...
class Button:
    ...
    def reset(self):
        self.clicked = False
        self.color = self.inactive_color# text
class Text:
    def __init__(self, text, font_size, color, position):
        self.text = text
        self.font_size = font_size
...
class PlayerSheet:
    def __init__(self, screen):
        ...
        self.nameLabel = Text("Name:", 36, BLACK, (10, 10))
        self.name_textbox = pygame_textinput.TextInputVisualizer(font_color=BLACK,cursor_color=BLUE)
        self.strengthLabel = Text("Hit Points:", 36, BLACK, (10, 40))
        self.Resume = Button(50, 540, 100, 50, "Resume", RED, BLACK)

    def draw(self):
        self.nameLabel.render(self.screen)
        self.strengthLabel.render(self.screen)

        if len(self.name_textbox.value) < self.character_limit:
            self.name_textbox.value = self.name_textbox.value
        else:
            self.name_textbox.value = self.name_textbox.value[:self.character_limit]
        
        self.screen.blit(self.name_textbox.surface, (90,10))
    ...
    def handle_event(self, event):
        pygame_widgets.update(event)
        self.name_textbox.update(event)
        for event in event:
